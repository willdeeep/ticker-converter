...
class CleanedMarketData(BaseModel):
    ...
    cleaning_applied: List[str] = Field(default_factory=list, description="Applied cleaning operations")
    outliers_removed: int = Field(default=0, description="Number of outliers removed")
    missing_values_filled: int = Field(default=0, description="Number of missing values filled")
...
class FeatureEngineeredData(BaseModel):
    ...
    features_dataframe: Optional[bytes] = Field(None, description="Serialized DataFrame with features")
    ...
    # Feature metadata
    features_created: List[str] = Field(default_factory=list, description="List of created features")
    processed_at: datetime = Field(default_factory=datetime.utcnow)
