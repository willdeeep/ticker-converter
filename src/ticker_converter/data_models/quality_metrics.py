...
class DataQualityMetrics(BaseModel):
    ...
    complete_records: int = Field(default=0, ge=0, description="Records with no missing values")
    missing_value_count: int = Field(default=0, ge=0, description="Total missing values")
    duplicate_count: int = Field(default=0, ge=0, description="Number of duplicate records")
    outlier_count: int = Field(default=0, ge=0, description="Number of statistical outliers")
    
    # Data type consistency
    type_consistency_score: float = Field(default=1.0, ge=0, le=1, description="Data type consistency (0-1)")
    ...
    date_gaps: int = Field(default=0, ge=0, description="Number of missing dates in sequence")
    
    # Price data validation
    negative_prices: int = Field(default=0, ge=0, description="Number of negative price values")
    zero_volume_days: int = Field(default=0, ge=0, description="Days with zero trading volume")
    price_anomalies: int = Field(default=0, ge=0, description="Price values outside expected ranges")
    
    # Calculated quality scores
    completeness_score: float = Field(default=0.0, ge=0, le=1, description="Data completeness (0-1)")
    consistency_score: float = Field(default=0.0, ge=0, le=1, description="Data consistency (0-1)")
    validity_score: float = Field(default=0.0, ge=0, le=1, description="Data validity (0-1)")
    overall_quality_score: float = Field(default=0.0, ge=0, le=1, description="Overall quality score (0-1)")
    ...
    def calculate_scores(self) -> None:
        ...
        self.consistency_score = max(0.0, self.type_consistency_score - consistency_penalty)
...
class DataQualityReport(BaseModel):
    ...
    field_completeness: Dict[str, float] = Field(default_factory=dict, description="Completeness by field")
    field_validity: Dict[str, float] = Field(default_factory=dict, description="Validity by field")
    
    # Statistical summaries
    price_statistics: Dict[str, float] = Field(default_factory=dict, description="Price field statistics")
