...
class AirflowManager:
    ...
    def _run_airflow_command(self, command: list[str], capture_output: bool = True) -> dict[str, Any]:
    ...
    def create_admin_user(self) -> dict[str, Any]:
        ...
        try:
            self.logger.info("Creating Airflow admin user: %s", self.admin_config["username"])
            ...
            if user_result.get("success"):
                ...
                results["success"] = True
                self.logger.info("Admin user created successfully: %s", self.admin_config["username"])
            else:
                ...
                if "already exists" in stderr.lower():
                    ...
                    results["success"] = True
                    self.logger.info("Admin user already exists: %s", self.admin_config["username"])
                else:
    ...
    def start_webserver(self, port: int = 8080, background: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            if background:
                webserver_result = self._run_airflow_command(webserver_command, capture_output=False)
                
                if webserver_result.get("success"):
                    results["webserver_started"] = True
                    results["success"] = True
                    results["pid"] = webserver_result.get("pid")
                    self.logger.info("Airflow webserver started in background (PID: %s)", results["pid"])
                ...
                webserver_result = self._run_airflow_command(webserver_command, capture_output=True)
                results["success"] = webserver_result.get("success", False)
                if not results["success"]:
                    results["errors"].append(webserver_result.get("stderr", "Unknown error"))
    ...
    def start_scheduler(self, background: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            if background:
                scheduler_result = self._run_airflow_command(scheduler_command, capture_output=False)
                
                if scheduler_result.get("success"):
                    results["scheduler_started"] = True
                    results["success"] = True
                    results["pid"] = scheduler_result.get("pid")
                    self.logger.info("Airflow scheduler started in background (PID: %s)", results["pid"])
                ...
                # For foreground execution (blocking)
                scheduler_result = self._run_airflow_command(scheduler_command, capture_output=True)
                results["success"] = scheduler_result.get("success", False)
                if not results["success"]:
                    results["errors"].append(scheduler_result.get("stderr", "Unknown error"))
            ...
            try:
                ...
                if ps_result.returncode == 0:
                    for line in ps_result.stdout.split('\n'):
                        if 'airflow' in line and ('webserver' in line or 'scheduler' in line):
                            parts = line.split()
                            if len(parts) > 1:
                                try:
                                    pid = int(parts[1])
                                    process_type = 'webserver' if 'webserver' in line else 'scheduler'
                ...
                for pid, process_type in processes_found:
                    try:
                        ...
                        results["services_stopped"].append(f"{process_type} (PID: {pid})")
                        self.logger.info("Stopped %s process (PID: %d)", process_type, pid)
                    except Exception as e:
                        error_msg = f"Failed to stop {process_type} (PID: {pid}): {str(e)}"
                        results["errors"].append(error_msg)
    ...
    def setup_airflow_complete(self) -> dict[str, Any]:
        ...
        try:
            ...
            results["operations_completed"].extend(db_results.get("operations_completed", []))
            ...
            results["success"] = (
                ...
                user_results.get("success", False)
            )
    ...
    def teardown_airflow(self) -> dict[str, Any]:
        ...
        try:
            ...
            if stop_results.get("success"):
                ...
                if services_stopped:
                    results["operations_completed"].append(f"stopped_services: {', '.join(services_stopped)}")
                else:
