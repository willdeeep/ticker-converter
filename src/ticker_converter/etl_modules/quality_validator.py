...
class ValidationConfig(BaseModel):
    ...
    check_completeness: bool = Field(default=True, description="Check data completeness")
    ...
    # Thresholds
    min_completeness_score: float = Field(default=0.95, description="Minimum completeness score")
    min_consistency_score: float = Field(default=0.90, description="Minimum consistency score")
    min_validity_score: float = Field(default=0.95, description="Minimum validity score")
    
    # Price validation
    max_daily_price_change: float = Field(default=0.20, description="Maximum daily price change (20%)")
...
class QualityValidator:
    ...
    def generate_quality_report(self, df: pd.DataFrame, symbol: str) -> DataQualityReport:
        ...
        logger.info(f"Quality report generated for {symbol}. Overall score: {metrics.overall_quality_score:.3f}")
    ...
    def _check_consistency(self, df: pd.DataFrame, result: ValidationResult) -> None:
        ...
        try:
            ...
            if all(col in df.columns for col in ['High', 'Low', 'Close', 'Open']):
                ...
                if invalid_close > 0:
                    result.add_error(f"Found {invalid_close} records where Close is outside High/Low range")
            
            # Check for consistent symbol
            if 'Symbol' in df.columns:
                unique_symbols = df['Symbol'].nunique()
                if unique_symbols > 1:
                    result.add_warning(f"Found {unique_symbols} different symbols in dataset")
        ...
        try:
            ...
            for col in price_columns:
                ...
                if negative_prices > 0:
                    result.add_error(f"Found {negative_prices} non-positive values in {col}")
                ...
                if min_val < self.config.min_price_value:
                    result.add_warning(f"{col} minimum value {min_val} below threshold {self.config.min_price_value}")
                
                if max_val > self.config.max_price_value:
                    result.add_warning(f"{col} maximum value {max_val} above threshold {self.config.max_price_value}")
                
                # Check for extreme daily changes
                if len(df) > 1:
                    daily_changes = df[col].pct_change(fill_method=None).abs()
                    extreme_changes = (daily_changes > self.config.max_daily_price_change).sum()
            ...
            if 'Volume' in df.columns:
                ...
                if extreme_volume > 0:
                    result.add_warning(f"Found {extreme_volume} extremely high volume values")
    ...
    def _check_timeliness(self, df: pd.DataFrame, result: ValidationResult) -> None:
        ...
        try:
            ...
            # Check for data gaps
            if len(df) > 1:
                expected_business_days = pd.bdate_range(start=df.index.min(), end=df.index.max())
                missing_days = len(expected_business_days) - len(df)
                
                if missing_days > 0:
                    result.add_warning(f"Found {missing_days} missing business days in date range")
    ...
    def _calculate_quality_metrics(self, df: pd.DataFrame) -> DataQualityMetrics:
        ...
        if isinstance(df.index, pd.DatetimeIndex):
            ...
            if len(df) > 1:
                expected_business_days = pd.bdate_range(start=df.index.min(), end=df.index.max())
                date_gaps = max(0, len(expected_business_days) - len(df))  # Ensure non-negative
        ...
        for col in price_columns:
            if col in df.columns:
                ...
                if len(df) > 1:
                    extreme_changes = (df[col].pct_change(fill_method=None).abs() > self.config.max_daily_price_change).sum()
    ...
    def _analyze_field_quality(self, df: pd.DataFrame, report: DataQualityReport) -> None:
    ...
    def _analyze_statistical_properties(self, df: pd.DataFrame, report: DataQualityReport) -> None:
    ...
    def _generate_recommendations(self, metrics: DataQualityMetrics, report: DataQualityReport) -> None:
        ...
        if metrics.date_gaps > 5:
            report.add_recommendation("Consider filling gaps in date sequence for time series analysis")
        
        if metrics.price_anomalies > metrics.total_records * 0.05:
            report.add_recommendation("Review and potentially filter extreme price movements")
        
        if metrics.overall_quality_score < 0.8:
            report.add_recommendation("Data quality is below acceptable threshold - comprehensive cleaning required")
