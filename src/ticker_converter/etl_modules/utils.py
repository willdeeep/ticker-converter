...
class PriceValidator:
    ...
    @staticmethod
    def validate_price_relationships(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, int]]:
        ...
        required_cols = [MarketDataColumns.HIGH, MarketDataColumns.LOW, MarketDataColumns.CLOSE]
        ...
        if invalid_high_low.any():
            ...
            logger.warning(f"Removed {removal_counts['high_low_violations']} rows with High < Low")
        
        # Remove rows where Close is outside High/Low range
        invalid_close = (
            (df[MarketDataColumns.CLOSE] > df[MarketDataColumns.HIGH]) | 
        ...
        for col in MarketDataColumns.PRICE_COLUMNS:
            if col in df.columns:
                ...
                if negative_prices.any():
                    ...
                    logger.warning(f"Removed {count} rows with non-positive {col} prices")
        
        return df, removal_counts
    
    @staticmethod
    def check_price_validity(df: pd.DataFrame, config: Optional[Dict] = None) -> Dict[str, int]:
        ...
        for col in MarketDataColumns.PRICE_COLUMNS:
            ...
            if len(df) > 1:
                ...
                violations['extreme_changes'] += (daily_changes > config['max_daily_change']).sum()
...
class DataFrameUtils:
    ...
    def ensure_numeric_types(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if MarketDataColumns.VOLUME in df.columns:
            df[MarketDataColumns.VOLUME] = df[MarketDataColumns.VOLUME].fillna(0).astype('int64')
    ...
    def remove_outliers_from_dataframe(
        ...
        for col in columns:
            ...
            if method == "iqr":
                ...
                col_outliers = OutlierDetector.detect_outliers_zscore(df[col], threshold)
            else:
...
class FeatureEngineering:
    ...
    @staticmethod
    def calculate_returns(df: pd.DataFrame, price_col: str = MarketDataColumns.CLOSE) -> pd.DataFrame:
    ...
    @staticmethod
    def calculate_moving_average(df: pd.DataFrame, column: str, period: int) -> pd.Series:
    ...
    def calculate_true_range(df: pd.DataFrame) -> pd.Series:
        ...
        high_close_prev = np.abs(df[MarketDataColumns.HIGH] - df[MarketDataColumns.CLOSE].shift(1))
        low_close_prev = np.abs(df[MarketDataColumns.LOW] - df[MarketDataColumns.CLOSE].shift(1))
