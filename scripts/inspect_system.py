    ...
    try:
        ...
        if postgres_password:
            try:
                ...
                connection.close()
                diagnostics["components"]["postgresql"] = {"status": "✅ Connected", "details": "PostgreSQL (business data) connection successful"}
            except ImportError:
                diagnostics["components"]["postgresql"] = {"status": "❌ Driver Missing", "details": "PostgreSQL driver (psycopg2) not available"}
            except Exception as conn_err:
                diagnostics["components"]["postgresql"] = {"status": "❌ Unavailable", "details": f"PostgreSQL connection failed: {str(conn_err)}"}
        else:
            diagnostics["components"]["postgresql"] = {"status": "⚠️  Not Configured", "details": "PostgreSQL credentials not configured in environment"}
        ...
        try:
            ...
            if os.path.exists(airflow_db_path):
                ...
                connection.close()
                diagnostics["components"]["airflow_db"] = {"status": "✅ Connected", "details": "Airflow SQLite database accessible"}
            else:
                diagnostics["components"]["airflow_db"] = {"status": "❌ Missing", "details": "Airflow database file not found (run 'make airflow' to initialize)"}
        except Exception as airflow_err:
            diagnostics["components"]["airflow_db"] = {"status": "❌ Error", "details": f"Airflow database error: {str(airflow_err)}"}
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if "Error Message" in data:
                            diagnostics["components"]["api"] = {"status": "❌ Invalid Key", "details": "API key authentication failed"}
                        elif "Note" in data:
                            diagnostics["components"]["api"] = {"status": "⚠️  Rate Limited", "details": "API key valid but rate limited"}
                        else:
                            diagnostics["components"]["api"] = {"status": "✅ Connected", "details": "API key valid and responsive"}
                    else:
                        diagnostics["components"]["api"] = {"status": "⚠️  Service Issue", "details": f"API responded with status {response.status_code}"}
                except ImportError:
                    diagnostics["components"]["api"] = {"status": "✅ Configured", "details": "API key configured (requests module not available for testing)"}
                except Exception as api_err:
                    diagnostics["components"]["api"] = {"status": "✅ Configured", "details": f"API key configured (connectivity test failed: {str(api_err)})"}
            else:
                diagnostics["components"]["api"] = {"status": "✅ Configured", "details": "API key configured (use --detailed for connectivity test)"}
    except Exception as e:
