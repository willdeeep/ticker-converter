...
class TestAlphaVantageClient:
    ...
    def test_client_initialization_without_api_key_raises_error(self):
        ...
        with patch("src.ticker_converter.api_clients.api_client.get_api_config") as mock_get_config:
            mock_get_config.side_effect = ValueError("ALPHA_VANTAGE_API_KEY environment variable is required")
    ...
    def test_make_request_success(self, mock_sleep, alpha_vantage_client, sample_daily_response):
        ...
        with patch.object(alpha_vantage_client.session, "get", return_value=mock_response):
            result = alpha_vantage_client.make_request({"function": "TIME_SERIES_DAILY"})

        assert result == sample_daily_response
        mock_sleep.assert_called_once_with(12)  # Rate limiting (12 seconds for free tier)
    ...
    def test_get_daily_stock_data_success(self, alpha_vantage_client, sample_daily_response):
        ...
        with patch.object(alpha_vantage_client, "make_request", return_value=sample_daily_response):
    ...
    def test_get_daily_stock_data_unexpected_format(self, alpha_vantage_client):
        ...
        with patch.object(alpha_vantage_client, "make_request", return_value=invalid_response):
    ...
    def test_get_intraday_stock_data_success(self, alpha_vantage_client, sample_intraday_response):
        ...
        with patch.object(alpha_vantage_client, "make_request", return_value=sample_intraday_response):
    ...
    def test_get_company_overview_success(self, alpha_vantage_client, sample_company_overview):
        ...
        with patch.object(alpha_vantage_client, "make_request", return_value=sample_company_overview):
    ...
    def test_symbol_case_handling(self, alpha_vantage_client, sample_daily_response):
        ...
        with patch.object(alpha_vantage_client, "make_request", return_value=sample_daily_response) as mock_request:
            alpha_vantage_client.get_daily_stock_data("aapl")
