...
class TestFinancialDataPipeline:
    ...
    def test_fetch_stock_data_compact_period(
        self, financial_pipeline, sample_dataframe
    ):
        """Test fetching stock data with compact period."""
        with patch.object(
            financial_pipeline.alpha_vantage,
    ...
    def test_get_company_info_success(
        self, financial_pipeline, sample_company_overview
    ):
        """Test successful company info retrieval."""
        with patch.object(
            financial_pipeline.alpha_vantage,
            "get_company_overview",
            return_value=sample_company_overview,
        ) as mock_get:
