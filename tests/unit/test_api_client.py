...
from src.ticker_converter.api_client import AlphaVantageClient, AlphaVantageAPIError
...
class TestAlphaVantageClient:
    ...
    def test_client_initialization_without_api_key_raises_error(self, mock_config):
        ...
        with pytest.raises(AlphaVantageAPIError, match="Alpha Vantage API key is required"):
            AlphaVantageClient()
    ...
    @patch('time.sleep')
    def test_make_request_success(self, mock_sleep, alpha_vantage_client, sample_daily_response):
        ...
        with patch.object(alpha_vantage_client.session, 'get', return_value=mock_response):
            result = alpha_vantage_client._make_request({"function": "TIME_SERIES_DAILY"})
        
        assert result == sample_daily_response
        mock_sleep.assert_called_once_with(1.0)  # Rate limiting
    
    @patch('time.sleep')
    def test_make_request_api_error(self, mock_sleep, alpha_vantage_client):
        ...
        with patch.object(alpha_vantage_client.session, 'get', return_value=mock_response):
            with pytest.raises(AlphaVantageAPIError, match="API Error: Invalid API call"):
                alpha_vantage_client._make_request({"function": "TIME_SERIES_DAILY"})
    
    @patch('time.sleep')
    def test_make_request_rate_limit_retry(self, mock_sleep, alpha_vantage_client, sample_daily_response):
        ...
        with patch.object(alpha_vantage_client.session, 'get', 
    ...
    @patch('time.sleep')
    def test_make_request_max_retries_exceeded(self, mock_sleep, alpha_vantage_client):
        """Test max retries exceeded."""
        with patch.object(alpha_vantage_client.session, 'get', 
                         side_effect=requests.RequestException("Connection error")):
            with pytest.raises(AlphaVantageAPIError, match="Request failed after 3 attempts"):
                alpha_vantage_client._make_request({"function": "TIME_SERIES_DAILY"})
    
    def test_get_daily_stock_data_success(self, alpha_vantage_client, sample_daily_response):
        """Test successful daily stock data retrieval."""
        with patch.object(alpha_vantage_client, '_make_request', return_value=sample_daily_response):
    ...
    def test_get_daily_stock_data_unexpected_format(self, alpha_vantage_client):
        ...
        with patch.object(alpha_vantage_client, '_make_request', return_value=invalid_response):
            with pytest.raises(AlphaVantageAPIError, match="Unexpected response format"):
                alpha_vantage_client.get_daily_stock_data("AAPL")
    
    def test_get_intraday_stock_data_success(self, alpha_vantage_client, sample_intraday_response):
        """Test successful intraday stock data retrieval."""
        with patch.object(alpha_vantage_client, '_make_request', return_value=sample_intraday_response):
            df = alpha_vantage_client.get_intraday_stock_data("AAPL", "5min")
        
        assert isinstance(df, pd.DataFrame)
        assert len(df) == 2
        assert list(df.columns) == ['DateTime', 'Open', 'High', 'Low', 'Close', 'Volume', 'Symbol']
        assert df['Symbol'].iloc[0] == 'AAPL'
    
    def test_get_company_overview_success(self, alpha_vantage_client, sample_company_overview):
        """Test successful company overview retrieval."""
        with patch.object(alpha_vantage_client, '_make_request', return_value=sample_company_overview):
    ...
    def test_symbol_case_handling(self, alpha_vantage_client, sample_daily_response):
        ...
        with patch.object(alpha_vantage_client, '_make_request', return_value=sample_daily_response) as mock_request:
            alpha_vantage_client.get_daily_stock_data("aapl")
