...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_flow(self, stock_data, storage_type, tmp_path):
        ...
        assert len(loaded_data) == len(stock_data), "Loaded data should have same number of records"
    ...
    def test_cross_format_compatibility(self, stock_data, tmp_path):
        ...
        assert len(json_loaded) == len(parquet_loaded), "Both formats should have same record count"
        assert list(json_loaded.columns) == list(parquet_loaded.columns), "Both formats should have same columns"
        ...
        try:
            ...
            if len(data) > 1000:  # Only run if we got a substantial dataset
                ...
                for storage_type in ["json", "parquet"]:
                    ...
                    assert save_time < 30, f"{storage_type} save took too long: {save_time}s"
                    assert load_time < 30, f"{storage_type} load took too long: {load_time}s"
            else:
