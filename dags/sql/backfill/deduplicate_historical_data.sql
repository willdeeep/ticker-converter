...
AND created_at IN (
    ...
    INNER JOIN stock_duplicates AS sd ON sp.symbol = sd.symbol AND sp.date = sd.date
    WHERE sd.rn > 1
);
...
AND created_at IN (
    ...
    INNER JOIN exchange_duplicates AS ed ON
...
(
    ...
    FORMAT('Deduplication completed for backfill period %s to %s',
           '{{ params.start_date }}',  -- noqa: TMP
           '{{ params.end_date }}')    -- noqa: TMP
);
